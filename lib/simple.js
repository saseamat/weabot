...
class WAConnection {
    ...
    async downloadMediaMessage(message, fileName) {
        ...
        if (fileName) {
            ...
            let trueFileName = fileName ? ('./temp/' + fileName + '.' + ftype.ext || mimetype.split("/")[1]) : getRandom(ftype.ext || mimetype.split("/")[1])
    ...
    async downloadAndSaveMediaMessage(message, filename, attachExtension = true) {
        ...
        let trueFileName = attachExtension ? ('./temp/' + filename + '.' + type.ext) : filename
        // save to file
